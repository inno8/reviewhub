...
# stored encrypted in the LLMConfiguration table. The resolver in
# users/org_llm.py:_platform_llm_config() reads from the DB.
#
# ai-engine reads PR_LLM_MODEL + COMMIT_LLM_MODEL from its own env to
# pick the right model per task tier (haiku for commit-level inline
# reviews, sonnet for full-PR rubric grading). The DB-stored model
# is a fallback for any code path that doesn't go through tier
# routing.
