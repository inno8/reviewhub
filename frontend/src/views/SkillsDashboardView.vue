...
const PATTERN_DESCRIPTIONS: Record<string, string> = {
  'clean_code': 'Clean Code betekent code schrijven die makkelijk te lezen, te begrijpen en te onderhouden is. Denk aan consistente naamgeving, nette opmaak, onnodige complexiteit vermijden en je houden aan taal-specifieke style guides (bijvoorbeeld PEP 8 voor Python). Clean code voorkomt bugs, omdat andere ontwikkelaars (en jijzelf over een half jaar) in een oogopslag zien wat de code doet.',
  'code_structure': 'Code Structure draait om het organiseren van je code in logische, modulaire stukken. Goede structuur betekent verantwoordelijkheden scheiden, functies gebruiken voor herbruikbare logica, bestanden gefocust houden op één taak en diep geneste code vermijden. Goed gestructureerde code is makkelijker te testen, te debuggen en uit te breiden.',
  'dry_principle': 'DRY (Don\'t Repeat Yourself) betekent dat je codeduplicatie voorkomt. Zie je dezelfde logica op meerdere plekken terugkomen? Trek het dan naar een gedeelde functie of module. Bij gedupliceerde code moet je elke bugfix of wijziging op meerdere plekken doorvoeren, wat de kans op inconsistenties en bugs vergroot.',
  'input_validation': 'Invoervalidatie betekent dat je alle data uit externe bronnen (gebruikersinvoer, API-requests, bestandsuploads) controleert en opschoont voordat je ze gebruikt. Zonder validatie is je applicatie kwetsbaar voor injection-aanvallen (SQL-injection, XSS), crashes door onverwachte datatypes en lekken in de veiligheid.',
  'error_handling': 'Error handling betekent dat je voorziet wat er fout kan gaan en code schrijft die daar netjes mee omgaat. Denk aan try/except-blokken, retourwaarden valideren, edge cases afvangen (lege lijsten, null-waarden) en duidelijke foutmeldingen geven in plaats van stilletjes crashen.',
  'edge_cases': 'Edge cases zijn ongebruikelijke of extreme invoer die je code mogelijk niet correct afhandelt — lege strings, nullen, hele grote getallen, null/undefined of onverwachte datatypes. Testen op edge cases voorkomt bugs die pas in productie opduiken met echte data.',
  'html_semantics': 'Semantische HTML betekent dat je de juiste HTML-elementen gebruikt voor hun bedoelde doel — header, nav, main, article, button in plaats van generieke div-tags met click-handlers. Semantische HTML verbetert toegankelijkheid (schermlezers), SEO en maakt je code leesbaarder en beter te onderhouden.',
  'accessibility': 'Toegankelijkheid (a11y) betekent dat je je webapplicatie bruikbaar maakt voor iedereen, óók voor mensen met een beperking. Denk aan alt-teksten bij afbeeldingen, een correcte koppenhiërarchie, werkende toetsenbordnavigatie, ARIA-labels op interactieve elementen en voldoende kleurcontrast.',
  'css_organization': 'CSS-organisatie betekent dat je je stylesheets schoon houdt, inline styles vermijdt, consistente naamgeving gebruikt (BEM, utility classes) en dubbele regels elimineert. Goed georganiseerde CSS gebruikt variabelen voor kleuren en spacing, groepeert gerelateerde stijlen en vermijdt specificity-gevechten.',
  'responsive_design': 'Responsive design betekent dat je layouts bouwt die zich aanpassen aan verschillende schermgroottes — mobiel, tablet en desktop. Daarbij gebruik je relatieve units (rem, %), media queries, flexbox/grid en test je op meerdere apparaten. Een responsive site biedt een goede ervaring ongeacht de schermgrootte.',
  'xss_csrf_prevention': 'XSS-preventie (Cross-Site Scripting) betekent dat je nooit data van gebruikers in HTML zet zonder die eerst op te schonen. Gebruik je innerHTML met gebruikersdata, dan kan een aanvaller malicieuze scripts injecteren. Gebruik in plaats daarvan textContent, template literals met goede escaping of een sanitization-library.',
  'database_queries': 'Veilige database-queries betekent dat je geparametriseerde queries (prepared statements) gebruikt in plaats van SQL te bouwen met string-concatenatie. Concatenatie maakt SQL-injection mogelijk, waarbij een aanvaller je query kan aanpassen om data in te zien, te wijzigen of te verwijderen die niet voor zijn ogen bedoeld is.',
  'secrets_management': 'Secrets management betekent dat je wachtwoorden, API-keys, tokens en andere gevoelige waarden nooit hardgecodeerd in je broncode zet. Gebruik in plaats daarvan environment variables, een secret manager (zoals AWS Secrets Manager) of .env-bestanden (uitgesloten van git). Hardgecodeerde geheimen in je git-history kunnen door iedereen worden gevonden en misbruikt.',
  'comments_docs': 'Comments en documentatie betekent: duidelijke inline-comments bij complexe logica, docstrings bij functies met uitleg over parameters en retourwaarden, en goed onderhouden README-bestanden. Goede documentatie helpt anderen (en jezelf later) de codebase te begrijpen zonder elke regel code te hoeven lezen.',
  'solid_principles': 'De SOLID-principes zijn vijf ontwerpprincipes voor onderhoudbare object-georiënteerde code: Single Responsibility (één class = één taak), Open/Closed (open voor uitbreiding, gesloten voor wijziging), Liskov Substitution, Interface Segregation en Dependency Inversion. SOLID volgen vermindert koppeling en maakt code makkelijker aan te passen.',
  'api_design': 'API-design betekent dat je consistente, intuïtieve REST-endpoints maakt met de juiste HTTP-methodes (GET om te lezen, POST om aan te maken, enzovoort), zinvolle statuscodes, duidelijke foutresponses, versionering en goede authenticatie. Een goed ontworpen API is makkelijk te gebruiken voor andere ontwikkelaars, zonder dat ze eindeloze documentatie hoeven te lezen.',
};
...
function openPatternInfo(patternKey: string) {
  ...
  patternInfoData.value = {
    name,
    slug,
    description: PATTERN_DESCRIPTIONS[slug] || `${name} is een terugkerend codekwaliteitspatroon dat in je commits is gedetecteerd. Bekijk de bijbehorende bevindingen op de Skills-pagina om te zien om welke problemen het gaat en hoe je ze oplost.`,
  } as any;
