        ...
        <section class="flex flex-col min-h-0" data-testid="comment-editor">
          ...
          <div class="p-4 flex-1 min-h-0 flex flex-col gap-3">
            ...
            <textarea
              ...
              v-model="draftBody"
              :disabled="!canEdit"
              class="flex-1 min-h-[180px] bg-surface-container-lowest border border-outline-variant/30 rounded-lg text-on-surface font-mono text-sm p-3 resize-none focus:ring-1 focus:ring-primary/50 focus:outline-none leading-relaxed disabled:opacity-60"
...
const isDirty = computed(() => draftBody.value !== originalBody);
...
function onSave() {
  ...
  emit('save', { ...props.comment, body: draftBody.value });
}
